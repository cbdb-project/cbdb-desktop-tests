...
_本報告產生於 2026-09-04 12:38 UTC，依據一次 253 項測試的執行結果（耗時 71 秒）。_
...
| 六個單次查詢的表單 | 49 | 入仕、官職、社會地位、著述、社會關係、地點——查詢與匯出 |
| 具狀態的表單 | 13 | 親屬關係、社會網路、關係配對、群組資料——工作清單 |
| 索引地址排序 | 9 | 唯一會改寫 CBDB 正式資料（而非暫存表）的端點 |
| 本報告自身的依據 | 19 | 以下每一項問題所引用的程式位置仍然存在，且中英文皆已填寫 |
...
這道指令會解開壓縮檔、以釋出資料庫的私有複本啟動釋出的執行檔、執行 253 項測試，並重新產生這幾份檔案。測試套件不會寫入作為對照基準的 `Data/CBDB.db`——每次執行都使用各自的複本，因此跑完之後，發行檔與執行前完全相同。
