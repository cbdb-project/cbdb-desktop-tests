...
_本報告產生於 2026-09-10 07:28 UTC，依據一次 1342 項測試的執行結果（耗時 407 秒）。_
...
在 100 項失敗中，有 **96** 項是用來證明下列問題的測試——這些問題正是由它們認定的，問題修好之後它們就會恢復通過。其餘 **4** 項在下方逐一交代，以免讀者拿這些數字去對照問題清單，卻發現兩邊對不起來。
...
| 本報告自身的依據 | 61 | 以下每一項問題所引用的程式位置仍然存在，且中英文皆已填寫 |
...
這道指令會解開壓縮檔、以釋出資料庫的私有複本啟動釋出的執行檔、執行 1342 項測試，並重新產生這幾份檔案。測試套件不會寫入作為對照基準的 `Data/CBDB.db`——每次執行都使用各自的複本，因此跑完之後，發行檔與執行前完全相同。
